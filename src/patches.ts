...
// One token per process, refreshed on expiry - deliberately not shared with
// DattoRmmClient's own AuthManager (that instance is per-request in gateway
// mode anyway, per mcp-server.ts's header comment on credential isolation),
// so this trades a little redundant token acquisition for staying fully
// decoupled from the SDK's private internals.
let cachedToken: { accessToken: string; expiresAt: number; apiUrl: string } | null = null;

async function getAccessToken(creds: DattoCredentials): Promise<string> {
  const apiUrl = PLATFORM_URLS[creds.platform];

  if (cachedToken && cachedToken.apiUrl === apiUrl && Date.now() < cachedToken.expiresAt - 5 * 60 * 1000) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to acquire Datto RMM token: ${response.status} ${response.statusText} - ${body}`);
  }

  const data = (await response.json()) as { access_token: string; expires_in: number };
  cachedToken = { accessToken: data.access_token, expiresAt: Date.now() + data.expires_in * 1000, apiUrl };
